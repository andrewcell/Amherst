...
function ConstructBoardString() {
    var ret = "#d즐겜요~ 여기 보이는 숫자만큼 찾으세요>#k#e->#k " + LeftToBeRevealed + "\r\n";
    var num = 0;
    for (var i = 0;i < NumCols;i++) {
        for (var j = 0;j < NumRows;j++) {
    ...
    if (mode == -1 || mode == 0)
        ...
        if (status == 0) {
            ...
            if (Revealed[Move[0]][Move[1]] == VALUE_REVEALED) { //Already went there
                SendStr += "이미 오픈 하신곳을 다시 오픈하신것 같은데요...?\r\n\r\n" + ConstructBoardString();
            ...
            if (Board[Move[0]][Move[1]] == MINE_ITEM) {
                //Lose
                cm.sendOk(Header() + ConstructEndBoardString() + "\r\n졌습니다!. 게임한시간: " + GetGameTime((new Date()).getTime()));
                cm.dispose();
            } else {
                if (IsGameOver()) {
                    cm.sendOk(Header() + ConstructEndBoardString() + "\r\n이겼습니다! 게임한시간: " + GetGameTime((new Date()).getTime()));
                    cm.gainMeso(30000)
                    cm.dispose();
                } else {
                    cm.sendSimple(SendStr);
