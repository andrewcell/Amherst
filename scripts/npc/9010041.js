...
var item = [[5520000,1500,1],[4220000, 900, 1], [4033919, 300, 1], [4170049, 500, 1], [3014000, 700, 1]];
var hitem = [[1302999,100,1],[1122017,80,1],[1112405,50,1],[2430218,30,1],[1112400,25,1],[4034218,20,1],[4001126,1,10],[4031456,10,1],[5060002,1,1]];
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.sendSimple("안녕하세요, 저는 이스트월드에서 회계직을 담당하는 미스 어카운트 라고 합니다. 사냥을 통해 적립된 헌팅포인트관련 업무를 집중적으로 하고있으니 많이 이용해주시기 바랍니다.#b\r\n#L0#헌팅포인트 상점 이용하기. #l\r\n#L2##r[HOT]#k #b홍보포인트 상점 이용하기. #l\r\n#L1#헌팅포인트를 메이플포인트로 환전하기. #l\r\n#L3#홍보포인트를 후원캐쉬로 환전하기. #l");
        status++;
    } else if (status == 1) {
        if (selection == 0) {
            ...
            cm.sendGetNumber("메이플포인트로 환전할 헌팅포인트의 값을 입력해주세요.\r\n#r(!)#k 2000 메이플 포인트당 100 헌팅포인트.", 0, 0, 1000000);
            status = 99;
        } else if (selection == 3) {
        ...
        if (mode != 1) {
            ...
            if (epp >= selection) {
                ...
                cm.getPlayer().modifyCSPoints(2, selection*20, true);
                cm.sendOk(selection  + "헌팅포인트를 환산하여 #b" + selection*20 + "#k 메이플 포인트를 획득하셨습니다.");
                cm.dispose();
            } else {
        ...
        cm.sendOk("#b#i" + h + "# #t" + h + "##k를 구매합니다.\r\n구매하지 않으시려면 esc를 눌러주세요.");
        status = 89;
