...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        var str = "NPC 입니다. 원하시는 항목을 선택하세요." +
                ...
                //"#L13##t4031119# -> 20,000 캐시 교환#l\r\n"+
                //"#L14##t4031119# -> 스킬명 이펙트 교환#l\r\n"
                //        + "#L6#Debug#l"
                "";
        //"#L3#서버 안내#l\r\n"+
        if (cm.getPlayer().hasGmLevel(6))
            str += "#L1934#아이템전송#l\r\n";
        str +=
                "#L19##e" + cashpricetext2 + " 메소당 1회 캐시 교환#n#l\r\n" +
                "#L5##z4031120# 교환 (후원 보상)#l\r\n" +
                "#L4##z4031120# 안내 (후원 안내)#l\r\n" +
                "#L7#성형, 헤어#l\r\n" +
                "#L1886#부화기 구매#l\r\n" +
                "#L20#캐시장비템에 마우스를 갖다대면 튕겨요..#l\r\n";
        //        }


        if (cm.getPlayer().isSuperGM()) {
            str += "#L6#아이템지급#l\r\n";
            str += "#L10#메용준다#l\r\n";
            str += "#L113#ㅁㄴㅇㄹ#l\r\n";
            str += "#L114#all채금#l\r\n";
            str += "#L115#all채금off#l\r\n";
            str += "#L116#날아오르라 주작이여#l\r\n";
            str += "#L121#날아오르라 주작이여2#l\r\n";
            str += "#L117#다 일루와#l\r\n";
            str += "#L118#노래방송#l\r\n";
        }
        ...
        if (selection == 0) {
            ...
            cm.sendOk("잠시 오류가 있어 현재 사용하실 수 없습니다.");
            cm.dispose();
            return;
            ...
            cm.sendNext("#e MapleStory Server#n\r\n"
                    ...
                    + "\r\n"
                    + "현재 접속하신 서버는 #b스카니아#k 서버 이며, #r2007년 당시 본서버를 그대로 모방하여 만든#k 서버입니다.\r\n"
                    + "\r\n"
                    + "사용할 수 있는 모든 기능은 100% 사용가능하며, 미구현 항목은 없습니다. 개인상점, 미니게임, 커플링 등 각종 서브컨텐츠 또한 100% 구현되어 마음대로 사용하실 수 있습니다.\r\n"
                    + "\r\n"
                    + "아무쪼록 저희 서버에서 즐거운 시간 보내시기 바라며, 서버가 마음에 드셨다면 지속적인 운영을 위해 자율적인 기금을 후원해주시면 감사하겠습니다. 기금 후원 안내는 이 항목 아래의 #b[후원 안내] 항목#k을 선택하시면 보실 수 있습니다. 굳이 기금을 후원해주시지 않아도 저희 서버를 즐기시는데는 아무런 지장이 없습니다.\r\n"
                    + "\r\n"
                    );
            status = -1;
        } else if (selection == 4) {
            cm.sendNext("#e후원 안내#n\r\n"
                    + "\r\n"
                    + "매달 지출되는 서버 호스팅 임대비용, 서버에 위해를 끼치는 공격등을 방어하기 위한 서버 유지비용에 대한 자율적인 후원을 받고 있습니다.\r\n"
                    + "후원은 자신의 선택이며, 후원을 하지 않으셔도 캐시를 포함하여 저희 서버를 즐기시는데\r\n"
                    + "아무런 제한이 없습니다. 또한 후원을 하신다고 #e#r절대#k#n 스탯이 높은 아이템을 지급하거나, \r\n"
                    + "게임머니를 지급하거나 다른 사람에 비하여 빠른 레벨업을 제공해 드리지 않습니다. \r\n"
                    + "\r\n"
                    + "또한 여러분의 후원금은 거의 100%라고 봐도 좋을만큼 서버 운영 유지비에 모두 사용됩니다.\r\n"
                    + "절대 개인 이윤을 위해 후원을 받는것이 아님을 이해해 주시기 바랍니다.\r\n"
                    + "\r\n"
                    + "후원을 해주신 분께는 감사의 의미로 서버 운영자로서 드릴 수 있는 소정의 보상인 #b#i4031120# #t4031120##k을 드립니다. \r\n"
                    + "\r\n"
                    + "#i4031120# #t4031120#은 KMS 1.2.182 버전까지 새로 추가된 본섭 성형, 헤어, 코디아이템을 구매할 수 있는 캐시로 교환할 수 있습니다.\r\n"
                    + "아래와 같은 아이템을 비롯하여 여러가지 본섭 코디아이템을 구매할 수 있습니다.\r\n"
                    + "\r\n"
                    + "#d#v1002953# #z1002953# #v1012083# #z1012083# #v1051189# #z1051189# #v1072384# #z1072384# #v1102142# #z1102142##k\r\n" //인형핑크 노란설레임 블링블링 슈즈 화염 망토
                    + "\r\n"
                    ...
                    + "\r\n"
                    + "#d히나인형은 메일이 확인된 후, 택배 NPC <듀이> 를 통하여 수령하실 수 있습니다.#k\r\n"
                    + "\r\n"
                    );
            status = -1;
        } else if (selection == 5) {
            ...
            if (chr != null) {
                ...
                try {
                    ...
                    while (cIter.hasNext())
                    {
                        var ch = cIter.next();
                        if (aitem != -1)
                            Packages.server.MapleItemInformationProvider.getInstance().getItemEffect(aitem).applyTo(ch);
                        else
                        if (ch.getMorphState() != 2210000) {
                            ch.addHP(-32767);
                        }
                    }
                } catch (e)
