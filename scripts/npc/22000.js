...
function action(mode, type, selection){
	if(status == 0){
		cm.sendYesNo("안녕하세요 #r#e이스트월드#n#k 에 입문하신것을 환영합니다. 저희는 항상 색다른 패치와 최상의 서비스로 #b#e"+cm.getPlayer().getName()+"#n#k 님에게 다가갈것을 약속드립니다. 자 여행의 준비가 완료되셨다면 확인을 눌러주세요.");
		status++;
	} else if(status == 1) {
	...
	if (selection == 0) {
		...
		cm.getPlayer().ServerNotice("바실리 : "+cm.getPlayer().getName()+"(남)님이 이스트 시작의 배에 탑승하였습니다. 모두 환영해주세요.")
		...
		cm.TimeMoveMap(490000001, 100000000, 90);
		cm.getPlayer().ServerNotice("바실리 : "+cm.getPlayer().getName()+"(여)님이 이스트 시작의 배에 탑승하였습니다. 모두 환영해주세요.")
		cm.sendOk("모험을 시작하기전 몇가지 중요한 #rTIP#k을 알려드립니다.\r\n#b1.#k캐쉬샵 버튼을 누르면 광장으로이동할수있다.\r\n#b2.#k헤네시스 택시앞에 초기자금퀘스트가 있다.\r\n#b3.#k@랙 , @저장 , @주문의흔적 유저 명령어가있다.");
		cm.dispose();
		}
	}
